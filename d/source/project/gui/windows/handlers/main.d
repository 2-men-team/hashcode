...
final class OpenFileButtonPressed : OnClickHandler, Attachable {
  /// Handler method
  ...
  override Attachable attachTo(Widget button) pure nothrow @safe {
    button.click = this;
    return this;
  }
}
...
final class SaveFileButtonPressed : OnClickHandler, Attachable {
  /// Handler mathod
  override bool onClick(Widget src) @system {
    // dialog to show
...
final class ProceedButtonPressed : OnClickHandler, Attachable {
  /// Handler method
  override bool onClick(Widget src) @system {
    // input data
    ...
    try { // try execute the main algorithm
      Simulator.instance.initialize(data).exec;

      // data to show on 'score' window
      Attachable[string] content = [
        "scoreText": new TextHolder(UIString.fromRaw(Simulator.instance.score.to!dstring))
      ];
